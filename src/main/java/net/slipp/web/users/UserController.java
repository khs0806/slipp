...
public class UserController {
	...
	public String joinDo(@Valid User user, BindingResult result) {
		log.debug("User : {}", user);
		if (result.hasErrors()) {
			log.debug("Result has error!");
			List<ObjectError> errors =  result.getAllErrors();
			for (ObjectError error : errors) log.debug("error : {}", error.getDefaultMessage());
			return "users/form";
		}
		userDao.create(user);
	...
	public String loginDo(@Valid Authenticate authenticate,BindingResult result,Model model, HttpSession session) {
